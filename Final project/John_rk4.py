...
class particles:
    def __init__(self,npart=10000,n=1000,soft=1,periodic=True):
        ...
        if periodic:
            ...
            self.pot=np.zeros([self.ngrid,self.ngrid])
        else:
    ...
    def take_step(self,dt=1):
        self.x[:]=rk4(self.x,self.v,dt)
        self.get_pot()
        self.get_forces()
        self.v[:]=rk4(self.v,self.f,dt)


np.random.seed(2)
num_part = 100000
grid_size = 1000
parts=particles(npart=num_part, n=grid_size,soft=1,periodic=True)
...
parts.x = np.array([x, y]).transpose()
